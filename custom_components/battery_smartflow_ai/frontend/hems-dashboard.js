        ...
        : `<section class="section"><div class="section-head"><h2>${this._escape(this._t("topology"))}</h2><small>${systems.length} ${this._escape(this._t("systems"))} · ${packCount} ${this._escape(this._t("packs"))}</small></div>
          ${systems.length ? `<div class="systems">${systems.map((system) => {
            ...
            const systemDetails = [
              ...
              [this._t("last_data"), system.data_age_seconds == null ? "—" : `${system.data_age_seconds}s`],
            ...
            const packCards = (system.packs || []).map((pack, index) => {
              ...
              return `<article class="pack-card" tabindex="0"><strong>↳ ${this._escape(this._t("battery_pack"))} ${packNumber}</strong><small>${this._escape(pack.model || this._t("zendure_pack"))}</small><aside class="hover-details"><h3>${this._escape(this._t("battery_pack"))} ${packNumber}</h3>${packRows || `<div class="detail-row"><span>${this._escape(this._t("telemetry"))}</span><strong>${this._escape(this._t("unavailable"))}</strong></div>`}</aside></article>`;
            }).join("");
            return `<article class="system-card" tabindex="0"><div class="system-head"><div><strong>${this._escape(system.name || this._t("system"))}</strong><small>${this._escape(system.model || system.profile || this._t("zendure_hardware"))}</small></div><span class="status ${system.online ? "" : "offline"}">${this._escape(this._t(system.online ? "online" : "offline"))}</span></div><div class="device-meta">${this._escape(system.transport || this._t("unknown_path"))} · ${this._escape(system.status || this._t("unknown_status"))}</div><div class="packs">${packCards || `<div class="device-meta">${this._escape(this._t("no_packs"))}</div>`}</div><aside class="hover-details"><h3>${this._escape(system.name || this._t("system"))}</h3>${systemDetails}${this._detailRows(systemEntities)}</aside></article>`;
