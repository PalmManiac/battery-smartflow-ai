...
class BatterySmartFlowDashboard extends HTMLElement {
  ...
  _formatAge(seconds) {
    if (!Number.isFinite(Number(seconds)) || seconds < 0) return this._t("unavailable");
  ...
  _healthSummary(systems) {
    ...
    const ages = systems.map((system) => Number(system.data_age_seconds)).filter((age) => Number.isFinite(age) && age >= 0);
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        *{box-sizing:border-box}.shell{max-width:1500px;margin:auto;padding:28px clamp(18px,3vw,42px) 56px}
        ...
        @media(max-width:900px){.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.topology{grid-template-columns:1fr}.link{height:20px;width:1px;margin:auto}}@media(max-width:520px){header{align-items:flex-start;flex-direction:column}.metrics{grid-template-columns:1fr 1fr}.systems{grid-template-columns:1fr}.health-grid{grid-template-columns:1fr}.section-head{align-items:flex-start;flex-direction:column}.metric{padding:13px}.section{padding:15px}}
